...
\begin{document}
	...
	\begin{enumerate}
		...
		\item Kalman
		\begin{verse}
		Filtr Kalmana został oparty na bibliotek "\texttt{The KFilter Project}"\footnote{http://kalman.sourceforge.net/}. Filtr skonfigurowany jest na potrzeby procesu ruchu o zmiennym przyśpieszeniu w przestrzeni dwu wymiarowej. Bierze pod uwagę szumy procesu, oraz szumy pomiarowe. Parametry filtra są definiowane w plikach ini: (początkowe położenie, początkowa prędkość oraz szum procesu i szum pomiarowy).  Filtr operuje na wektorze stanu procesu zawierającym położenie oraz wektor prędkości. Dla każdej współrzędnej został stworzony oddzielny obiekt filtru działający tylko w jednym wymiarze. Jako wejście obiekt filtru dostaje także przyśpieszenie będące sterowaniem obiektu. Przyśpieszenie to jest przekazywane z generatora. 
		\end{verse}
		\item Writer
		\begin{verse}
